...
public class HooksStepDefs {
    ...
    static {
        ...
        hooksOrder.add("@BeforeFeature(order = 10)");
        hooksOrder.add("@BeforeFeature(@CleanServerData)");
        hooksOrder.add("@Before");
        hooksOrder.add("@After");
        hooksOrder.add("@Before");
        hooksOrder.add("@After");
        hooksOrder.add("@AfterFeature(@RestoreServerData)");
        hooksOrder.add("@AfterFeature(order = 10)");
    ...
    @BeforeFeature("@CleanServerData")
     public void cleanServerData(){
        ensureHooksOrder("@BeforeFeature(@CleanServerData)");
        assertThat(httpServer.receive(), is(notNullValue()));
        dataToRestore = httpServer.receive();
        httpServer.clean();
    }

    @AfterFeature("@RestoreServerData")
    public void restoreServerData(){
        ensureHooksOrder("@AfterFeature(@RestoreServerData)");
        assertThat(httpServer.receive(), is(notNullValue()));
        httpServer.send(new HttpServerStub.Request<String>(dataToRestore));
    }

    @BeforeFeature(order = 10)
    public void beforeFeature(){
        ensureHooksOrder("@BeforeFeature(order = 10)");
    }

    @AfterFeature(order = 10)
    public void afterFeature(){
        ensureHooksOrder("@AfterFeature(order = 10)");
    }
