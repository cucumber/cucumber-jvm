...
A simple cuke4duke maven plugin for running your features. Written by Øystein Ellingbø (ellingbo) and Ole Christian Rynning (oc) :-)
 ...
 * Added cuke4duke-webtest-example, which should illustrate some pretty basic running of web-(integration)tests with the plugin. (Remember to set your jrubyHome!).
...
If you wish to refer to <code>mvn cuke4duke:features</code> instead of <code>mvn cuke4duke.mojo:cuke4duke:features</code>,
...
<settings>
  ...
  <pluginGroups>
    <pluginGroup>cuke4duke.mojo</pluginGroup>
  </pluginGroups>
  ...
</settings>
</pre>

<code>mvn cuke4duke:features -Dcucumber.features=examples/simple/features -Djruby.home=/opt/jruby-1.2.0 -Dcucumber.installGems=true</code>
...
<project>
  ...
  <build>
    <plugins>
      <plugin>
        <groupId>cuke4duke.mojo</groupId>
...
By default it scans for features in <code>features</code>.
...
<project>
 ...
 <build>
        <finalName>cuke4duke-webtest-example</finalName>
        <plugins>
            <plugin>
                <groupId>cuke4duke.mojo</groupId>
                <artifactId>cuke4duke-maven-plugin</artifactId>
