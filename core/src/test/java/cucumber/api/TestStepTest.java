...
public class TestStepTest {
    ...
    public void run_wraps_run_step_in_test_step_started_and_finished_events() throws Throwable {
        step.run(bus, language, scenario, false, true);
    ...
    public void run_does_dry_run_step_when_skip_steps_is_true() throws Throwable {
        step.run(bus, language, scenario, true, false);
    ...
    public void result_is_passed_when_step_definition_does_not_throw_exception() {
        Result result = step.run(bus, language, scenario, false, false);

        assertEquals(Result.Type.PASSED, result.getStatus());
    }

    @Test
    public void result_is_skipped_when_skip_step_is_true() {
        Result result = step.run(bus, language, scenario, true, false);
