...
//
// private static final Comparator<TimelineFormatter.TestData>
// TEST_DATA_COMPARATOR = Comparator
// .comparing(o -> o.scenario);
//
// private static final String REPORT_TEMPLATE_RESOURCE_DIR =
// "src/main/resources/io/cucumber/core/plugin/timeline";
//
...
// .create();
//
// private final Feature failingFeature =
// TestFeatureParser.parse("some/path/failing.feature", "" +
...
// " Then step_03");
//
// private final Feature successfulFeature =
// TestFeatureParser.parse("some/path/successful.feature", "" +
...
// " Then step_30");
//
// private final Feature pendingFeature =
// TestFeatureParser.parse("some/path/pending.feature", "" +
...
//
// final List<String> files = Arrays.asList("index.html", "formatter.js",
// "jquery-3.5.1.min.js",
...
// assertThat(e + ": did not exist in output dir", actualFile.exists(),
// is(equalTo(true)));
// final String actual = readFileContents(actualFile.getAbsolutePath());
// final String expected = readFileContents(new
// File(REPORT_TEMPLATE_RESOURCE_DIR, e).getAbsolutePath());
// assertThat(e + " differs", actual, is(equalTo(expected)));
// }
// }
//
// private void runFormatterWithPlugin() {
// StepDurationTimeService timeService = new
// StepDurationTimeService(Duration.ofMillis(1000));
//
// Runtime.builder()
...
// final Scanner scanner = new Scanner(new FileInputStream(outputPath),
// "UTF-8");
...
// readFileContents(reportJsFile.getAbsolutePath()).split("\n");
...
// groupLines.append("]");
//
// final TimelineFormatter.TestData[] tests =
// gson.fromJson(itemLines.toString(),
// TimelineFormatter.TestData[].class);
// final TimelineFormatter.GroupData[] groups =
// gson.fromJson(groupLines.toString(),
...
//
// assertAll(
// () -> assertThat(String.format("feature on item %s, was not as expected",
// idx), actual.feature,
// is(equalTo(expected.feature))),
// () -> assertThat(String.format("className on item %s, was not as expected",
// idx), actual.className,
// is(equalTo(expected.className))),
// () -> assertThat(String.format("content on item %s, was not as expected",
// idx), actual.content,
...
// assertAll(
// () -> assertThat(String.format("startTime on item %s, was not as expected",
// idx),
// actual.startTime, is(equalTo(expected.startTime))),
// () -> assertThat(String.format("endTime on item %s, was not as expected",
// idx),
...
//
// ActualReportOutput(final TimelineFormatter.TestData[] tests, final
// TimelineFormatter.GroupData[] groups) {
