...
public final class SubstitutionSteps {
    ...
    void I_have_a_user_account_with_my_name(String name) {
        this.name = name;
    }

    @When("an Admin grants me {word} rights")
    void an_Admin_grants_me_role_rights(String role) {
        this.role = role;
        this.details = ROLES.get(role);
    }

    @Then("I should receive an email with the body:")
    void I_should_receive_an_email_with_the_body(String body) {
        String expected = """
                Dear %s,
