...
GherkinDialectProvider.DIALECTS.keySet().each { language ->
    ...
    if (!unsupported.contains(normalized_language)) {
        ...
        def deprecatedTemplateSource = new File(project.baseDir, "src/main/groovy/deprecated-lambda.java.gsp").getText()
        def deprecatedTemplate = engine.createTemplate(deprecatedTemplateSource).make(binding)
        def deprecatedFile = new File(project.baseDir, "target/generated-sources/i18n/java/cucumber/api/java8/${className}.java")
