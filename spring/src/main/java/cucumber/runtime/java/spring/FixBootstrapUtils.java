...
/**
 * Copied from {@code org.springframework.test.context.BootstrapUtils} to allow  for thread local
 * instances of the <em>Spring TestContext Framework</em>. This is a work around to allow step
 * definitions to be added/removed from the spring context.
 * <p>
 * Ideally we don't add/remove step definitions to the spring context but that decision was made
 * ages ago and will need some time to be resolved.
 *
 * @author Sam Brannen
 * @author Phillip Webb
 * @see BootstrapWith
...
abstract class FixBootstrapUtils {
    ...
    private static Class<?> resolveDefaultTestContextBootstrapper(Class<?> testClass) throws ClassNotFoundException {
