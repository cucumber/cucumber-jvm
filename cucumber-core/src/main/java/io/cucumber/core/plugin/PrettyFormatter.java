...
public final class PrettyFormatter implements ConcurrentEventListener, ColorAware {
    ...
    public PrettyFormatter(OutputStream out) {
        this.out = out;
        this.writer = new MessagesToPrettyWriter(out, ansi());
    }
     ...
     */
    private static class MessagesToPrettyWriter implements AutoCloseable {
        ...
        private void printArgument(PickleStep pickleStep) {
            pickleStep.getArgument().ifPresent(pickleStepArgument -> {
                ...
                pickleStepArgument.getDataTable().ifPresent(pickleTable -> {
                    ...
                    try {
                        tableFormatter.formatTo(table, writer);
                    } catch (IOException e) {
                ...
                pickleStepArgument.getDocString().ifPresent(pickleDocString -> {
                    ...
                    try {
                        docStringFormatter.formatTo(docString, writer);
                    } catch (IOException e) {
                        throw new CucumberException(e);
                    }
                });
    ...
    private static class PrettyReportData {
        ...
        private StepDefinition updateStepDefinitionsById(StepDefinition stepDefinition) {
            return stepDefinitionsById.put(stepDefinition.getId(), stepDefinition);
        }
