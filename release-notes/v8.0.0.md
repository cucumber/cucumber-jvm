...
Nullability information through JSpecify
----------------------------------------
...
Notably when dealing with `io.cucumber.datatable.DataTable` consider declaring a
[`@DataTableType` transform](https://github.com/cucumber/cucumber-jvm/tree/main/cucumber-java#data-table-type)
and using the transformed argument rather than using `DataTable::asList` and
friends.
...
fun givenTheFollowingGroceries(table: DataTable){
    val groceries: List<Grocery> = table.asList(Grocery::class.java)
...
If you are using these plugins, when upgrading ensure there is a dependency on
Jackson 2:
...
Removed Java EE support in favor of Jakarta EE
----------------------------------------------
