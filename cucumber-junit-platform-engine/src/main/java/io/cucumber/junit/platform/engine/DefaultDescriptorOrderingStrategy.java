...
enum DefaultDescriptorOrderingStrategy implements DescriptorOrderingStrategy {
    ...
    RANDOM {
        ...
        private long createRandomSeed() {
            long generatedSeed = new Random().nextLong(Long.MAX_VALUE);
            log.config(() -> "Using generated seed for configuration parameter '%s' with value '%s'."
                    .formatted(EXECUTION_ORDER_RANDOM_SEED_PROPERTY_NAME, generatedSeed));
